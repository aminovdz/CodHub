...
export async function POST(req: NextRequest) {
  try {
    ...
    if (META_APP_SECRET && signature) {
      const expectedSig = 'sha256=' + crypto.createHmac('sha256', META_APP_SECRET).update(rawBody).digest('hex');
      if (signature !== expectedSig) {
        return NextResponse.json({ error: 'Invalid signature' }, { status: 401 });
      }
    } else if (!META_APP_SECRET) {
      console.warn('META_APP_SECRET is not set. Webhook signature verification is bypassed.');
    }
