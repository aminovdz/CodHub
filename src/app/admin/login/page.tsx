      ...
      <form onSubmit={handleLogin} className={`p-8 rounded-3xl shadow-2xl max-w-sm w-full border transition-colors duration-300 ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
        ...
        <div className="mb-6">
          ...
          <input
            ...
            className={`w-full px-4 py-4 rounded-xl border focus:ring-2 focus:ring-indigo-500 outline-none text-center tracking-widest font-mono text-xl transition-colors ${isDark ? 'bg-slate-900 border-slate-600 text-white placeholder:text-slate-600' : 'bg-slate-50 border-slate-300 text-slate-900'}`}
            placeholder="••••••••"
            autoComplete="off"
            required
          />
