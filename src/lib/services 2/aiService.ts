...
`;
    ...
    const isCreateLandingPage = prompt.toLowerCase().includes('landing page') || prompt.toLowerCase().includes('landingpage') || agentType.toLowerCase().includes('cro');

    const previewDataInstructions = isCreateLandingPage
...
For other action types: "previewData" can be any relevant data object.`
      : `"previewData": {} // Include any data needed for the UI to show a preview (e.g. product fields, analysis results)`;
